...
import { Order, Inventory, OrderStatus, PaymentStatus, MENU_ITEMS, cn } from "../lib";
import { 
  TrendingUp, 
  Package, 
  DollarSign, 
...
export function ManagerView({ orders, inventory, updatePaymentStatus, resetDay, setupInventory, location, setLocation }: ManagerViewProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-sunburst overflow-hidden relative">
      ...
      <div className="p-4 md:p-6 bg-white/10 backdrop-blur border-b-2 border-white/30 flex items-center justify-between flex-shrink-0 relative z-10">
        ...
        <div className="flex bg-white/20 p-1 rounded-xl overflow-x-auto border-2 border-white/40">
          {(['dashboard', 'inventory', 'orders', 'summary', 'settings'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={cn(
                "px-4 md:px-6 py-2.5 md:py-2 min-h-[44px] rounded-lg text-sm font-bold capitalize transition-all touch-target flex-shrink-0",
                activeTab === tab ? "bg-[#FFC107] text-[#B71C1C]" : "text-white/90 hover:text-white hover:bg-white/10"
              )}
            >
              {tab}
            </button>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-6 lg:p-8 relative z-10">
        {activeTab === "dashboard" && (
          <div className="space-y-8">
            {/* Top Stats */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              <StatCard title="Total Revenue" value={`${totalRevenue}฿`} icon={DollarSign} color="text-[#B71C1C]" bg="bg-[#FFC107]/30" />
              <StatCard title="Total Orders" value={paidOrders.length} icon={ShoppingCart} color="text-[#B71C1C]" bg="bg-[#FFC107]/30" />
              <StatCard title="Avg. Order" value={`${avgOrderValue.toFixed(0)}฿`} icon={TrendingUp} color="text-[#B71C1C]" bg="bg-[#FFC107]/30" />
              <StatCard title="Potatoes Left" value={inventory?.potatoStock || 0} icon={Package} color="text-[#B71C1C]" bg="bg-[#FFC107]/30" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Sales by Flavor */}
              <div className="bg-white/95 p-8 rounded-2xl shadow-xl border-2 border-white/60">
                <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2" style={{ fontFamily: "Fredoka, sans-serif" }}>
                  <BarChart3 className="w-5 h-5 text-[#B71C1C]" />
                  Sales by Flavor
                </h3>
              ...
              <div className="bg-white/95 p-8 rounded-2xl shadow-xl border-2 border-white/60">
                <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2" style={{ fontFamily: "Fredoka, sans-serif" }}>
                  <PieChart className="w-5 h-5 text-[#B71C1C]" />
                  Payment Breakdown
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {Object.entries(paymentBreakdown).map(([method, amount]) => (
                    <div key={method} className="bg-[#FFC107]/20 border-2 border-[#FFC107]/40 p-6 rounded-xl text-center">
        ...
        {activeTab === "summary" && (
          <div className="max-w-3xl mx-auto bg-white/95 p-12 rounded-2xl shadow-2xl border-2 border-white/60">
            <div className="text-center mb-12">
              <h3 className="text-4xl font-black text-[#B71C1C] mb-2" style={{ fontFamily: "Fredoka, sans-serif" }}>End of Day Summary</h3>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-sm">Business & Beat Booth Report</p>
              {location && (
                <p className="mt-4 flex items-center justify-center gap-2 text-slate-600 font-bold">
            ...
            <div className="grid grid-cols-2 gap-8 mb-12">
              <div className="space-y-6">
                <div className="border-b border-slate-100 pb-4">
                  ...
                  <p className="text-4xl font-black text-[#B71C1C]">{totalRevenue}฿</p>
                </div>
                <div className="border-b border-slate-100 pb-4">
            ...
            <button 
              ...
              className="w-full py-5 min-h-[56px] bg-[#B71C1C] text-white font-black rounded-2xl hover:bg-[#C62828] transition-all flex items-center justify-center gap-3 touch-target border-2 border-white/40"
            >
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="bg-white/95 p-8 rounded-2xl shadow-xl border-2 border-white/60">
              ...
              <div className="space-y-8">
                <div className="p-6 bg-slate-50 rounded-2xl">
                  ...
                  <div className="flex items-center gap-6">
                    <input 
                      ...
                      className="w-32 text-2xl md:text-3xl font-black p-4 rounded-xl border-2 border-slate-200 focus:border-[#FFC107] outline-none min-h-[48px] touch-target"
                    />
                      ...
                      <div className="h-4 bg-slate-200 rounded-full overflow-hidden">
                        <div 
                          className={cn(
                            ...
                            (inventory?.potatoStock || 0) <= (inventory?.thresholds.potato || 0) ? "bg-orange-500" : "bg-[#4CAF50]"
                          )}
                  ...
                  {MENU_ITEMS.map(item => (
                    <div key={item.id} className="p-6 bg-slate-50 rounded-2xl">
                      ...
                      <input 
                        ...
                        className="w-full text-xl md:text-2xl font-black p-3 rounded-xl border-2 border-slate-200 focus:border-[#FFC107] outline-none min-h-[48px] touch-target"
                      />
                ...
                <button 
                  onClick={handleSetup}
                  className="w-full py-4 min-h-[52px] bg-[#B71C1C] hover:bg-[#C62828] text-white font-bold rounded-2xl transition-all shadow-lg touch-target border-2 border-white/40"
                >
                  Update Inventory
                </button>
              </div>
        ...
        {activeTab === "orders" && (
          <div className="bg-white/95 rounded-2xl shadow-xl border-2 border-white/60 overflow-hidden">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#FFC107]/30 text-[#B71C1C] text-xs font-black uppercase tracking-widest border-b-2 border-white/40">
              ...
              <tbody className="divide-y divide-slate-100">
                {orders.slice().reverse().map(order => (
                  <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">{new Date(order.createdAt).toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{order.location || "—"}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        order.paymentStatus === 'paid' ? "bg-[#4CAF50]/30 text-[#2E7D32]" : "bg-slate-200 text-slate-600"
                      )}>
                        {order.paymentStatus}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                       <span className={cn(
                        "px-3 py-1 rounded-full text-xs font-black uppercase tracking-tighter",
                        order.status === 'completed' ? "bg-[#B71C1C] text-white" : "bg-[#FFC107]/40 text-[#B71C1C]"
                      )}>
                        {order.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {order.paymentStatus === 'paid' && (
                        <button 
                          ...
                          className="px-3 py-2 min-h-[44px] text-sm font-bold text-red-500 hover:bg-red-50 rounded-lg transition-colors touch-target"
                        >
                          Void
                        </button>
                      )}
                      {order.paymentStatus === 'unpaid' && (
                        <button 
                          ...
                          className="px-3 py-2 min-h-[44px] text-sm font-bold text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors touch-target"
                        >
                          Mark Paid
                        </button>
                      )}
        ...
        {activeTab === "settings" && (
          <div className="max-w-xl mx-auto space-y-8">
            <div className="bg-white/95 p-8 rounded-2xl shadow-xl border-2 border-white/60">
              <h3 className="text-xl font-bold text-slate-800 mb-6">Location</h3>
              <p className="text-sm text-slate-500 mb-4">Set the location where sales are recorded (e.g. Maharashtra, Booth A)</p>
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="flex-1 text-lg font-semibold p-4 rounded-xl border-2 border-slate-200 focus:border-[#FFC107] outline-none"
                />
              </div>
            </div>
             <div className="bg-white/95 p-8 rounded-2xl shadow-xl border-2 border-white/60">
              ...
              <div className="space-y-4">
                <button 
                  ...
                  className="w-full flex items-center justify-between p-4 min-h-[56px] bg-red-50 hover:bg-red-100 text-red-600 rounded-2xl transition-colors group touch-target"
                >
                  <div className="flex items-center gap-3">
...
function StatCard({ title, value, icon: Icon, color, bg }: any) {
  return (
    <div className="bg-white/95 p-6 rounded-2xl shadow-xl border-2 border-white/60 flex items-center gap-6">
      <div className={cn("w-14 h-14 rounded-xl flex items-center justify-center", bg)}>
        <Icon className={cn("w-7 h-7", color)} />
      </div>
      <div>
        <p className="text-sm font-bold text-slate-500 uppercase tracking-wider">{title}</p>
        <p className="text-3xl font-black text-slate-900" style={{ fontFamily: "Fredoka, sans-serif" }}>{value}</p>
      </div>
    </div>
  );
}
